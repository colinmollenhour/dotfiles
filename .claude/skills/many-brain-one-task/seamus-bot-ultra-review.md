...
- OpenCode with OpenAI/GPT-5.6 Sol at high reasoning effort (agent `colin-mbot-gpt-sol`; via OpenAI, not Zen). `mbot-run` now defaults `--variant high` and `--agent colin-mbot-gpt-sol` for GPT slots — do not omit them from the plan and do not use agent `build`. On an OpenCode host, `launch --detach` then `barrier`.
