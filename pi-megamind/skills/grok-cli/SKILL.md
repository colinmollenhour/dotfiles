...
grok models             # lists available models; default is usually grok-4.6
```
...
| `-m, --model <MODEL>` | Model id (default from config; often `grok-4.6`). Resolve with `grok models`. |
...
| Flag value | Notes |
| --- | --- |
| `grok-4.6` | Default strong coding / reasoning model |
