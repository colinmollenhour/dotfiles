...
interface SlotUsage {
  ...
  wall_source: "meta" | "agentsview" | "none"
  cost_usd: number | null
  cost_microdollars: number | null
  has_cost: boolean
  cost_source: "agentsview" | "unavailable" | "none"
...
function sessionIdCandidates(id: string): string[] {
...
function fetchUsage(sessionId: string, retries = 2): AgentsviewUsage | null {
  const keys = sessionIdCandidates(sessionId)
...
function fetchSessionGet(sessionId: string): AgentsviewSession | null {
  const keys = sessionIdCandidates(sessionId)
  for (const key of keys) {
...
`)
  ...
  for (const { path, meta } of metas) {
    ...
    const sessionId = fromMeta || fromFile
    ...
    const wall = wallSeconds(startedAt, endedAt)
    const wallSource: SlotUsage["wall_source"] = wall != null ? "meta" : "none"

    let usage: AgentsviewUsage | null = null
    if (agentsviewAvailable && sessionId) {
      usage = fetchUsage(sessionId)
      if (usage?.session_id) {
