...
- **Meta is source of truth for who worked:** write `results/<slot>.meta.json` (or `<out-basename>.meta.json`) **before or immediately after** each launch with at least: `slot`, `phase`, `planned_model`, `actual_model`, `display_name`, `provider_model_id`, `harness`, `backup_used`, `prompt`, `out`, `session_id`, `exit`, optional `supersedes`.

Scoring, Unique/Shared tables, and "who validated cluster X" **must** use `actual_model` from meta (or an updated plan entry) — never parse the performer from the `.out` basename alone.
...
1. Choose the backup from the profile (or built-in defaults). Do not invent substitutes.
